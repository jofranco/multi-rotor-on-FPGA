...
#(parameter
    ...
    input  wire                          ap_ready,
    input  wire                          ap_idle,
    output wire [15:0]                   min_duty,
    output wire [15:0]                   max_duty,
...
localparam
    ...
    ADDR_IER             = 6'h08,
    ADDR_ISR             = 6'h0c,
    ADDR_MIN_DUTY_DATA_0 = 6'h10,
    ADDR_MIN_DUTY_CTRL   = 6'h14,
    ADDR_MAX_DUTY_DATA_0 = 6'h18,
    ADDR_MAX_DUTY_CTRL   = 6'h1c,
    ADDR_PERIOD_DATA_0   = 6'h20,
    ADDR_PERIOD_CTRL     = 6'h24,
    ADDR_M_V_BASE        = 6'h30,
    ADDR_M_V_HIGH        = 6'h3f,
    WRIDLE               = 2'd0,
    WRDATA               = 2'd1,
...
//------------------------Local signal-------------------
    ...
    reg  [15:0]                   int_min_duty = 'b0;
    reg  [15:0]                   int_max_duty = 'b0;
    reg  [15:0]                   int_period = 'b0;
    // memory signals
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_MIN_DUTY_DATA_0: begin
                    rdata <= int_min_duty[15:0];
                end
                ADDR_MAX_DUTY_DATA_0: begin
                    rdata <= int_max_duty[15:0];
                end
                ADDR_PERIOD_DATA_0: begin
                    rdata <= int_period[15:0];
                end
            endcase
        end
...
assign min_duty  = int_min_duty;
assign max_duty  = int_max_duty;
assign period    = int_period;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
        int_ap_start <= 1'b0;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_MIN_DUTY_DATA_0)
            int_min_duty[15:0] <= (WDATA[31:0] & wmask) | (int_min_duty[15:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_MAX_DUTY_DATA_0)
            int_max_duty[15:0] <= (WDATA[31:0] & wmask) | (int_max_duty[15:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_PERIOD_DATA_0)
            int_period[15:0] <= (WDATA[31:0] & wmask) | (int_period[15:0] & ~wmask);
    end
end
