...
architecture behave of pid_INPUT_s_axi is
    ...
    component pid_INPUT_s_axi_ram is
        generic (
            BYTES   : INTEGER :=4;
            DEPTH   : INTEGER :=256;
            AWIDTH  : INTEGER :=8);
    ...
    end component pid_INPUT_s_axi_ram;
...
-- ----------------------- Instantiation------------------
-- int_cmdIn_V
int_cmdIn_V : pid_INPUT_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 3,
     AWIDTH   => log2(3))
port map (
     ...
     q1       => int_cmdIn_V_q1);
-- int_measured_V
int_measured_V : pid_INPUT_s_axi_ram
generic map (
     BYTES    => 4,
     DEPTH    => 3,
     AWIDTH   => log2(3))
...
-- ----------------------- AXI READ ----------------------
    ...
    RRESP   <= "00";  -- OKAY
    RVALID_t  <= '1' when (rstate = rddata) and (int_cmdIn_V_read = '0') and (int_measured_V_read = '0') else '0';
    RVALID    <= RVALID_t;
...
end entity pid_INPUT_s_axi_ram;

architecture behave of pid_INPUT_s_axi_ram is
