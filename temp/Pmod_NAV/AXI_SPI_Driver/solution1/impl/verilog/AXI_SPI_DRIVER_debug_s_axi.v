...
`timescale 1ns/1ps
module AXI_SPI_DRIVER_debug_s_axi
#(parameter
...
//------------------------Parameter----------------------
localparam
    ADDR_TX_MESSAGE_V_DATA_0 = 5'h10,
    ADDR_TX_MESSAGE_V_CTRL   = 5'h14,
    ADDR_RX_MESSAGE_V_DATA_0 = 5'h18,
    ADDR_RX_MESSAGE_V_CTRL   = 5'h1c,
    WRIDLE                   = 2'd0,
    WRDATA                   = 2'd1,
    WRRESP                   = 2'd2,
    WRRESET                  = 2'd3,
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (ar_hs) begin
            rdata <= 1'b0;
            case (raddr)
                ADDR_TX_MESSAGE_V_DATA_0: begin
                    rdata <= int_TX_message_V[31:0];
                end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_TX_message_V[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_TX_MESSAGE_V_DATA_0)
            int_TX_message_V[31:0] <= (WDATA[31:0] & wmask) | (int_TX_message_V[31:0] & ~wmask);
    end
end
